    ...
    return (
      <div className="relative grid w-full items-center gap-2">
        ...
        <div className="relative">
          <div
            className={cn(
              'flex h-[52px] w-full items-center overflow-hidden rounded-lg border', // add overflow-hidden
              error ? 'border-red-500' : 'border-[#D1D5DB]'
            )}
          >
            <RPNInput.default
              ref={ref}
              className="flex h-full w-full items-center placeholder:*:text-sm placeholder:*:font-normal placeholder:*:text-[#6B7280] focus:outline-none"
    ...
    <Popover modal={true}>
      <PopoverTrigger asChild>
        <Button
          ...
          variant={'ghost'}
          className={cn(
            'flex h-full gap-1 rounded-e-none rounded-s-sm border-r-[1px] border-r-[#D1D5DB] px-3',
      ...
      <PopoverContent
        ...
        collisionPadding={16}
        avoidCollisions
        className="w-[320px] rounded-xl border border-neutral-200 p-0 shadow-xl dark:border-neutral-800"
      >
        <Command>
          <CommandList>
